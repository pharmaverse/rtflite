...
for week in visit_weeks:
    ...
    for trt in ["Placebo", "Drug 25mg", "Drug 50mg"]:
        trt_data = df_vitals[
            (df_vitals["TREATMENT"] == trt) & (df_vitals["WEEK"] == week)
        ]["SBP"]
...
def get_analysis_formatting(row_index, row_data):
    ...
    # Endpoint descriptions
        ...
        analysis.startswith("Endpoint:")
        or analysis.startswith("Response Rate")
        or analysis.startswith("Time to Response")
        or analysis.startswith("Overall TEAE")
        or analysis.startswith("Most Common")
    ):
        return {
            "text_format": ["b", "b", "b", "b", "b"],
...
doc_analysis = rtf.RTFDocument(
    df=df_analysis,
    rtf_page=rtf.RTFPage(orientation="landscape", nrow=50),
    rtf_title=rtf.RTFTitle(
        text=[
