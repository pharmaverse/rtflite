...
df_fonts = pd.DataFrame(
    format_demo, columns=["Font Family", "Description", "Usage Example"]
)
...
doc_fonts = rtf.RTFDocument(
    ...
    rtf_title=rtf.RTFTitle(
        text=["Font Family Demonstration", "Various Typefaces and Applications"]
    ),
...
df_colors = pd.DataFrame(
    color_demo, columns=["Status", "Clinical Meaning", "Action Required"]
)
...
df_align = pd.DataFrame(
    align_demo, columns=["Endpoint", "Analysis Method", "Result Summary"]
)
...
symbols_demo = [
    ...
    [
        "Greek Letters",
        "α=0.05, β-blocker",
        "Statistical significance, medication types",
    ],
...
df_symbols = pd.DataFrame(
    symbols_demo, columns=["Category", "Examples", "Clinical Usage"]
)
...
df_lab_enhanced = pd.DataFrame(
    lab_enhanced, columns=["Test", "Result", "Reference Range", "Units", "Status"]
)

doc_lab = rtf.RTFDocument(
    df=df_lab_enhanced,
    rtf_page=rtf.RTFPage(orientation="portrait", nrow=50),
    rtf_title=rtf.RTFTitle(
        text=[
