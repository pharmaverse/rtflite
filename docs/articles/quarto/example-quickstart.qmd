...
- The `col_rel_width` in the `RTFBody()` function only control relative width of table body. Therefore the column header is not aligned as expected. More details will be provided in next example to take care of column header width.
- The total column width is defined in the `col_width` argument of the `RTFPage()` function.
...
If an empty column name is needed for a column, you can insert an empty string; e.g., `["name 1", "", "name 3"]`.
