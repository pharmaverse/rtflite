...
class PageConfig:
    ...
    forced_content: Set[str] = field(default_factory=set)  # Content IDs forced to this page
...
class PageBreakRule:
    ...
    def applies_to_row(self, df: pl.DataFrame, row_idx: int, prev_row_idx: Optional[int] = None) -> bool:
...
class PageDict(BaseModel):
    ...
    page_configs: Dict[int, PageConfig] = Field(
        ...
        description="Configuration for each page"
    )
    break_rules: List[PageBreakRule] = Field(
        default_factory=list,
        description="Rules determining where page breaks occur"
    )
    content_index: Dict[str, List[int]] = Field(
        default_factory=dict,
        description="Maps content identifiers to page numbers"
    )
    ...
    def calculate_pages_from_dataframe(
        ...
        if page_by and new_page:
            for col in page_by:
                self.add_break_rule(PageBreakRule(
                    column=col,
    ...
    def get_content_summary(self) -> Dict[str, Dict[str, Any]]:
        ...
        for content_id, page_num in self._content_assignments.items():
            summary[content_id] = {
                ...
                "is_forced": content_id in self.page_dict.page_configs.get(page_num, PageConfig(0, 0, 0, PageBreakType.AUTOMATIC)).forced_content
            }
        return summary
