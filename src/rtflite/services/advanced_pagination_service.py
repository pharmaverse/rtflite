...
from ..pagination import PageDict, PageIndexManager, PageBreakType
...
class AdvancedPaginationService:
    ...
    def create_page_dict(
        self, 
        document,
        nrow_per_page: Optional[int] = None
    ) -> PageDict:
    ...
    def _process_multi_section_document(
        ...
        for section_idx, df in enumerate(document.df):
            rtf_body = document.rtf_body[section_idx] if isinstance(document.rtf_body, list) else document.rtf_body
            
            # Create a temporary PageDict for this section
            section_page_dict = PageDict(nrow_per_page=page_dict.nrow_per_page)
            self._process_single_dataframe(df, rtf_body, section_page_dict, additional_rows)
    ...
    def validate_pagination(self) -> List[str]:
        ...
        for i in range(len(sorted_pages) - 1):
            ...
            if current_page.end_row >= next_page.start_row:
                issues.append(f"Pages {current_page.page_number} and {next_page.page_number} have overlapping row ranges")
