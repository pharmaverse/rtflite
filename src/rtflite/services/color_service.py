...
from rtflite.dictionary.color_table import color_table, name_to_type, name_to_rgb, name_to_rtf
...
class ColorService:
    ...
    def __init__(self):
        ...
        self._current_document_colors = None  # Context for current document being encoded
    ...
    def get_color_suggestions(self, partial_color: str, max_suggestions: int = 5) -> List[str]:
    ...
    def validate_color_list(self, colors: Union[str, List[str], Tuple[str, ...]]) -> List[str]:
        ...
        if isinstance(colors, str):
            ...
            raise ColorValidationError(f"Colors must be string, list, or tuple, got {type(colors)}")
        
        validated_colors = []
        for i, color in enumerate(colors):
            if not isinstance(color, str):
                raise ColorValidationError(f"Color at index {i} must be string, got {type(color)}")
    ...
    def needs_color_table(self, used_colors: Optional[List[str]] = None) -> bool:
        ...
        significant_colors = [color for color in used_colors if color and color != "black"]
    ...
    def generate_rtf_color_table(self, used_colors: Optional[List[str]] = None) -> str:
        ...
        if used_colors is None:
            ...
            filtered_colors = [color for color in used_colors if color and color != "black"]
    ...
    def get_rtf_color_index(self, color: str, used_colors: Optional[List[str]] = None) -> int:
    ...
    def collect_document_colors(self, document) -> List[str]:
        ...
        if document.rtf_body:
            bodies = [document.rtf_body] if not isinstance(document.rtf_body, list) else document.rtf_body
            for body in bodies:
        ...
        for component in components:
            if component:
                ...
                extract_colors_from_attribute(getattr(component, 'text_background_color', None))
            ...
            if isinstance(headers[0], list):
                ...
                for header_section in headers:
                    for header in header_section:
                        if header:
                            extract_colors_from_attribute(getattr(header, 'text_color', None))
                            extract_colors_from_attribute(getattr(header, 'text_background_color', None))
            else:
                # Flat format
    ...
    def set_document_context(self, document=None, used_colors: Optional[List[str]] = None):
