...
class TestRTFPageComponentPlacement:
    ...
    def test_rtfpage_validation(self, large_dataset):
        ...
        with pytest.raises(ValueError, match="Invalid page placement option"):
            RTFDocument(
                df=large_dataset,
                rtf_page=RTFPage(page_title="invalid")
            )
            
        with pytest.raises(ValueError, match="Invalid page placement option"):
            RTFDocument(
                df=large_dataset,
                rtf_page=RTFPage(page_footnote="invalid")
            )
            
        with pytest.raises(ValueError, match="Invalid page placement option"):
            RTFDocument(
                df=large_dataset,
                rtf_page=RTFPage(page_source="invalid")
            )
    ...
    def test_page_title_all(self, large_dataset):
        ...
        rtf_output = doc.rtf_encode()
        title_count = rtf_output.count('Test Title')
        assert title_count > 1, f"Expected title to appear multiple times, got {title_count}"
    ...
    def test_page_footnote_first(self, large_dataset):
        ...
        assert footnote_count == 1, f"Expected footnote to appear 1 time, got {footnote_count}"
    ...
    def test_page_footnote_all(self, large_dataset):
        ...
        rtf_output = doc.rtf_encode()
        footnote_count = rtf_output.count('Test Footnote')
        assert footnote_count > 1, f"Expected footnote to appear multiple times, got {footnote_count}"
    ...
    def test_page_source_first(self, large_dataset):
        ...
        assert source_count == 1, f"Expected source to appear 1 time, got {source_count}"
    ...
    def test_page_source_all(self, large_dataset):
        ...
        rtf_output = doc.rtf_encode()
        source_count = rtf_output.count('Test Source')
        assert source_count > 1, f"Expected source to appear multiple times, got {source_count}"
