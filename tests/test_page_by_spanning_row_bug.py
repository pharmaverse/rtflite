...
def test_page_by_should_create_spanning_rows_not_data_column():
    ...
    for i, line in enumerate(lines):
        ...
        if in_header and '\\cellx' in line:
            # Extract cellx value
            if 'cellx' in line:
    ...
    assert len(header_col_defs) == 2, (
        ...
        f"Bug: page_by column '__index__' should be removed from table, not kept as data column."
    )
    ...
    assert cell_count != 3, (
        f"First data row has 3 cells, which indicates page_by column is in the table as data. "
        f"Expected: spanning row (1 cell) or data row (2 cells). "
        f"Bug: page_by should create spanning rows, not keep column as data."
    )
    ...
    for line in lines[:100]:  # Check first 100 lines
        if '__index__' in line and '\\cell' in line:
            ...
            if line_idx > 0:
                ...
                if any('\\clvertalb' in l for l in nearby_lines):
