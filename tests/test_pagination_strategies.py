...
class TestPaginationBoundaryConditions:
    ...
    def test_one_row_over_page_boundary(self):
        ...
        doc = RTFDocument(
            ...
            rtf_column_header=RTFColumnHeader(text=["Row", "Value"]),
            rtf_footnote=RTFFootnote(text="Test footnote"),  # Add footnote for more rows
            rtf_body=RTFBody()
        )
...
class TestServiceLayerIntegration:
    ...
    def test_document_service_pagination_detection(self):
        ...
        doc_large = RTFDocument(
            df=df_large,
            rtf_page=RTFPage(nrow=20)
        )
    ...
    def test_grouping_service_restore_page_context(self):
        ...
        # Apply group_by
        transformed_df = service.enhance_group_by(
            original_df, ["Site", "Patient"]
        )
